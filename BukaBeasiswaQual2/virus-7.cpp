...
private:
	//Atribut virus
    string name;
    int malignancy, age, min, max, day;
    bool alive = true;
...
public:
    ...
    void Update() //Menampilkan output
    {
        ...
        if(alive) //Virus mati di usia 365 kecuali dengan nama yang diawali huruf D, virus mati tidak ditampilkan
		{
			...
			{
				if(name != "M4G3R" && name != "ABA81LV2" && name != "ABA81L" && name != "DARGOMBES" && name != "DEWARUCI" && name != "DOBBYTHEHOUSEELF" && name != "DOBBY") //M4G3R, ABA81L, DARGOMBES, DEWARUCI, DOBBYTHEHOUSEELF, DOBBY tidak terkena LebayEffect
				{
